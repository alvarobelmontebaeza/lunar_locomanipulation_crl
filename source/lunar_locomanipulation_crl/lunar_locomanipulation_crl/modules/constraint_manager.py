...
if TYPE_CHECKING:
    from envs import ConstrainedRlEnv
...
class ConstraintManager(ManagerBase):
    _env: ConstrainedRlEnv

    def __init__(
        self, cfg: object, env: ConstrainedRlEnv, tau: float = 0.95, min_p: float = 0.0
    ):
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> Dict[str, torch.Tensor]:
        ...
        for key in self._episode_sums.keys():
            ...
            violation_mean = (self._episode_sums[key][env_ids] / episode_lengths).mean(dim=0) * 100
            prob_mean = (self._cstr_mean_values[key][env_ids] / episode_lengths).mean(dim=0)
